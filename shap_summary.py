...
myfont = FontProperties(fname=r"c:\windows\fonts\simhei.ttf", size=12)##windows系统
######################################################################################

# TODO: remove unused title argument / use title argument
def summary_plot(shap_values, features=None, data_shap=None,feature_names=None, max_display=None, plot_type=None,
    ...
    if max_display is None:
        ############################################################
#        修改能够显示的最大的特征数量
#        max_display = 20  ##原来的代码
        max_display = len(shap_values.T)  ##添加的重构代码
    ...
    if color_bar and features is not None and plot_type != "bar" and \
        ...
        ##########################################################################################
#        print('labels',labels)  ##############添加的重构代码
        '''
        labels的含义与值
        labels {'MAIN_EFFECT': 'SHAP main effect value for\n%s', 'INTERACTION_VALUE': 'SHAP interaction value', 
        'INTERACTION_EFFECT': 'SHAP interaction value for\n%s and %s','VALUE': 'SHAP value (impact on model output)', 
        'GLOBAL_VALUE': 'mean(|SHAP value|) (average impact on model output magnitude)', 
        'VALUE_FOR': 'SHAP value for\n%s', 'PLOT_FOR': 'SHAP plot for %s', 'FEATURE': 'Feature %s',
        'FEATURE_VALUE': 'Feature value', 'FEATURE_VALUE_LOW': 'Low', 'FEATURE_VALUE_HIGH': 'High', 
        'JOINT_VALUE': 'Joint SHAP value', 'MODEL_OUTPUT': 'Model output value'}
        '''
        
        
        cb.set_ticklabels([labels['FEATURE_VALUE_LOW'], labels['FEATURE_VALUE_HIGH']])
        cb.set_label(color_bar_label, size=15, labelpad=0)  ##用来调节‘Feature value’的字体大小（原来的值是11）
        
        ###################################################################
#        修改‘条形颜色柱的HIGH、LOW’的大小
#        cb.ax.tick_params(labelsize=11, length=0)  ##原来的
        cb.ax.tick_params(labelsize=13, length=0)   ##添加的重构代码
        #########################################################################
        ...
        #######################################################################################
        ##该部分代码是用来调节colorbar的厚度
#        cb.ax.set_aspect((bbox.height - 0.9) * 20)    ##原先的
        cb.ax.set_aspect((bbox.height - 0.9) * 5)      ##添加的重构代码
    ...
    ####################################################################
    ##这里用来修改横坐标的范围，属于添加的重构内容
    pl.xlim(-30,50)  ##李烁的门诊量预测中的总数预测
    
    ####################################################################
    
    #######################################################
    ##用以修改不显示的中文特征名称
#    pl.yticks(range(len(feature_order)), [feature_names[i] for i in feature_order], fontsize=13)   #原来的代码
    pl.yticks(range(len(feature_order)), [feature_names[i] for i in feature_order], fontproperties=myfont) ##添加的重构代码
...
##################################################################################################
    ...
    if plot_type == "bar":
       ...
       ##########################################################
#    外加的重构代码
#    feature_order=[19 18 31 25  4  8 14 24 11  6  2  9 10  7  5 12  3  1 13  0]
    if data_shap is not None:
        myfont_num = FontProperties(fname=r"c:\windows\fonts\simhei.ttf", size=12)##windows系统
        for x,y in zip(range(len(feature_order)), data_shap['value']):
        # ha: horizontal alignment
        # va: vertical alignment
            pl.text(-30,x+0.05,'%.3f' % y,ha='center', va='bottom',fontsize=10)
#            pl.text(y-30, x+0.05, '%.5f' % y,ha='center', va='bottom',fontproperties=myfont_num)
 #############################################################################
